...
public interface UserRepository extends CrudRepository<User, UUID> {
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM public.users WHERE email = :email AND tenant_id = :tenantId)")
    boolean existsByEmailAndTenantId(String email, UUID tenantId);

    @Modifying
    @Query("UPDATE public.users SET status = :status, updated_at = :updatedAt WHERE id = :id")
    void updateStatus(UUID id, String status, Instant updatedAt);

    @Modifying
    @Query("UPDATE public.users SET email_verified = true, status = 'ACTIVE', updated_at = NOW() WHERE id = :userId")
    void verifyEmail(UUID userId);

    @Modifying
    @Query("UPDATE public.users SET tenant_id = :tenantId, updated_at = NOW() WHERE id = :userId")
    void assignTenant(UUID userId, UUID tenantId);

    @Modifying
    @Query("UPDATE public.users SET password_hash = :passwordHash, updated_at = NOW() WHERE id = :userId")
